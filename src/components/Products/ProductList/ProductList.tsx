...
import {useGetProductsByStore} from '../../../react-queries/product/productQueries';
...
const ProductList: React.FC<ProductListProps> = ({
  ...
  const {
    ...
    refetch,
    error,
  } = useGetProductsByStore({storeId, categoryId, limit});

  // Flatten all pages into a single array
  const products = data?.pages.flatMap(page => page.products) || [];

  const renderProduct: ListRenderItem<IProductTable> = useCallback(
    ({item}) => <ProductCard product={item} onPress={() => onProductPress?.(item)} />,
    [onProductPress],
  );
